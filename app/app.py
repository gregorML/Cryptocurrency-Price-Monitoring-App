...
with tab3:
    ...
    with col2:
        with st.container():
            ...
            threshold = st.slider("Monitoring Threshold (%):", min_value=float(0.01), max_value=float(5), value=float(1), step=float(0.1), key='threshold3')
            st.write(f"Selected threshold: {threshold:.3f}")
            save_config(threshold)
            threshold2 = st.slider("Display Threshold (%):", min_value=float(0.01), max_value=float(5), value=float(1), step=float(0.1), key='threshold32')
